<?php
$blog_id = get_option('page_for_posts');

if (is_category()) {
	$category    = get_the_category();
	$category_id = $category[0]->cat_ID;
} else {
	$category_id = false;
}

if (is_tag()) {
	$tag      = get_queried_object();
	$tag_slug = $tag->slug;
} else {
	$tag_slug = false;
}

?>


<?php global $wp_query; ?>
<?php get_header(); ?>

<main class="blog-page page-template">

    <section class="page-header">
        <div class="page-header__bg">
			<?php echo wp_get_attachment_image(get_field('bg', get_option('page_for_posts')), 'full'); ?>
        </div>
        <div class="container">
            <div class="page-header__row">

				<?php if (is_author()): ?>

                    <div class="author-widget">
                        <div class="author-photo">
                            <a href="<?php echo get_author_posts_url(get_the_author_meta('ID')) ?>">
								<?php echo get_avatar(get_the_author_meta('ID')); ?>
                            </a>
							<?php if (get_field('twitter', 'user_' . get_the_author_meta('ID'))): ?>
                                <a class="twitter-link" href="<?php echo get_field('twitter', 'user_' . get_the_author_meta('ID')) ?>" target="_blank">
                                    <svg aria-hidden="true" focusable="false" data-prefix="fab" data-icon="twitter" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-twitter fa-w-16 fa-3x">
                                        <path fill="currentColor"
                                              d="M459.37 151.716c.325 4.548.325 9.097.325 13.645 0 138.72-105.583 298.558-298.558 298.558-59.452 0-114.68-17.219-161.137-47.106 8.447.974 16.568 1.299 25.34 1.299 49.055 0 94.213-16.568 130.274-44.832-46.132-.975-84.792-31.188-98.112-72.772 6.498.974 12.995 1.624 19.818 1.624 9.421 0 18.843-1.3 27.614-3.573-48.081-9.747-84.143-51.98-84.143-102.985v-1.299c13.969 7.797 30.214 12.67 47.431 13.319-28.264-18.843-46.781-51.005-46.781-87.391 0-19.492 5.197-37.36 14.294-52.954 51.655 63.675 129.3 105.258 216.365 109.807-1.624-7.797-2.599-15.918-2.599-24.04 0-57.828 46.782-104.934 104.934-104.934 30.213 0 57.502 12.67 76.67 33.137 23.715-4.548 46.456-13.32 66.599-25.34-7.798 24.366-24.366 44.833-46.132 57.827 21.117-2.273 41.584-8.122 60.426-16.243-14.292 20.791-32.161 39.308-52.628 54.253z"
                                              class=""></path>
                                    </svg>
                                </a>
							<?php endif; ?>
                        </div>
                    </div>

                    <h1 class="h2 page-header__heading">
						<?php the_archive_title(); ?>
                    </h1>

				<?php else: ?>
                    <h3 class="page-header__subheading sub-heading uppercase">
						<?php echo get_field('subheading', get_option('page_for_posts')) ?>
                    </h3>

                    <h1 class="page-header__heading uppercase">
						<?php echo get_field('heading', get_option('page_for_posts')) ?>
                    </h1>

                    <div class="page-header__caption font-large">
						<?php echo get_field('caption', get_option('page_for_posts')) ?>
                    </div>
				<?php endif; ?>

            </div>
        </div>
    </section>

    <div class="blog-inner-wrap">
        <div class="insight-form-wrap">
            <div class="form">
				<?php echo do_shortcode('[contact-form-7 id="594" title="Get A Insights"]') ?>
            </div>
        </div>
        <div class="posts-grid__wrap">

			<?php if ( ! is_author()): ?>

                <div class="container">
                    <div class="filters">
                        <div class="filters__empty"></div>

                        <!-- Category Filter -->
                        <div class="category-filter">
                            <a class="<?php is_home() ? print 'active' : null; ?>" href="<?php echo get_permalink(get_option('page_for_posts')) ?>">All</a>
							<?php foreach (get_categories(['exclude' => '1']) as $category): ?>
                                <a class="<?php $category_id == $category->term_id ? print 'active' : null; ?>" href="<?php echo get_category_link($category->term_id) ?>"><?php echo $category->name; ?></a>
							<?php endforeach; ?>
                        </div>

                        <!-- Tag Filter -->
                        <div class="tag-filter">
							<?php foreach (get_tags([]) as $tag): ?>
                                <a class="<?php $tag_slug == $tag->slug ? print 'active' : null; ?>" href="<?php echo get_category_link($tag->term_id) ?>"><?php echo $tag->name; ?></a>
							<?php endforeach; ?>
                        </div>

                    </div>
                </div>

			<?php endif; ?>

            <div class="container">
                <div class="posts-grid">

					<?php if (have_posts()) :
						while (have_posts()) :
							the_post(); ?>

                            <article <?php post_class('post-template'); ?>>
                                <a href="<?php the_permalink(); ?>" class="post__thumbnail">
									<?php the_post_thumbnail(); ?>

									<?php if (get_field('main_category')): ?>
                                        <span class="main-category">
                                    <?php the_field('main_category'); ?>
                                </span>
									<?php endif; ?>

                                </a>
                                <div class="post__info">
                                    <div class="post__date"><?php echo get_the_date(); ?></div>
                                    <a href="<?php the_permalink(); ?>" class="post__permalink">
                                        <h2 class="post__title"><?php the_title() ?></h2>
                                    </a>
                                </div>
                            </article>

						<?php endwhile;
					endif; ?>
                </div>
                <div class="pagination-wrap">
					<?php if ($wp_query->max_num_pages > 1): ?>
						<?php paginator(get_pagenum_link(), $wp_query) ?>
					<?php endif; ?>
                </div>
            </div>
        </div>
    </div>

	<?php get_template_part('template-parts/sections/section-cta'); ?>
</main>

<?php get_footer(); ?>



