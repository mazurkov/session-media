...
load_theme_textdomain('prontomas', get_template_directory() . '/lang');
...
// add_theme_support()
add_action('after_setup_theme', 'prontomas_theme_support_settings');
function prontomas_theme_support_settings() {
	add_editor_style();
...
function prontomas_deregister_gutenberg_assets() {
...
add_action('wp_print_styles', 'prontomas_deregister_gutenberg_assets', 999);


// Deregister Gutenberg font for frontend
function prontomas_deregister_gutenberg_fonts($translation, $text, $context, $domain) {
...
if ( ! is_admin()) {
	add_filter('gettext_with_context', 'prontomas_deregister_gutenberg_fonts', 10, 4);
}


// Show plugin path in "wp-admin/plugins.php"
function prontomas_show_plugin_path($plugin_meta, $plugin_file) {
	echo '<code><small>' . esc_url($plugin_file) . '</small></code>  | ';

	return $plugin_meta;
}

add_filter('plugin_row_meta', 'prontomas_show_plugin_path', 10, 4);


function prontomas_remove_admin_login_header() {
	remove_action('wp_head', '_admin_bar_bump_cb');
}

add_action('get_header', 'prontomas_remove_admin_login_header');
